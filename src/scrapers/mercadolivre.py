"""
Scraper MercadoLivre - API publica oficial (sem OAuth para buscas).
...
def _scrape_model(model_name,query):
    try:
        resp=requests.get("https://api.mercadolibre.com/sites/MLB/search",params={"q":query,"limit":20,"condition":"new","category":MLB_CELULARES},headers=HEADERS,timeout=20)
        ...
        for item in resp.json().get("results",[])[:15]:
            ...
            seen.add(title)
            results.append({"store":"mercadolivre","model":model_name,"title":title[:120],"price":price,"url":item.get("permalink","https://www.mercadolivre.com.br/"),seller":(item.get("seller") or {}).get("nickname","MercadoLivre"),"product_id":f"ml_{item.get('id',abs(hash(title))%9999999)}"})
        return results
    except Exception as e:logger.warning(f"[ML] {e}");return []
