"""
Scraper Conecta — loja de tecnologia e smartphones (usa VTEX).
"""
import logging
import requests
...
def _scrape_model(model_name: str, query: str) -> list[dict]:
    try:
        resp = requests.get(
            "https://www.conecta.com.br/api/catalog_system/pub/products/search",
        ...
        for product in data[:5]:
            ...
            link = product.get("link", f"https://www.conecta.com.br/{product.get('linkText', '')}/p")
            results.append({
                "store": "conecta",
                "model": model_name,
                "title": title[:120],
                ...
                "url": link,
                "seller": "Conecta",
                "product_id": f"con_{product.get('productId', hash(title) % 999999)}",
            })

        return results

    except Exception as e:
        logger.warning(f"[Conecta] Erro ao buscar '{query}': {e}")
        return []
...
def get_prices() -> list[dict]:
    ...
    logger.info(f"[Conecta] {len(results)} ofertas encontradas.")
    return results
