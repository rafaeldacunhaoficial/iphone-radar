"""
Scraper B2W Group — Americanas, Submarino e Shoptime compartilham o mesmo backend.
"""
import logging
import re
import requests
...
STORES = [
    ("americanas", "Americanas", "americanas.com.br"),
    ("submarino", "Submarino", "submarino.com.br"),
    ("shoptime", "Shoptime", "shoptime.com.br"),
]
...
def _scrape_store(model_name, query, store_id, store_name, domain):
    try:
        ...
        if resp.status_code != 200:
            return []
        patterns = [
            r'"name"\s*:\s*"([^"]*(?:iPhone|iphone)[^"]*)"[^}]{0,300}?"bestPrice"\s*:\s*(\d+(?:\.\d+)?)',
            r'"productName"\s*:\s*"([^"]*(?:iPhone|iphone)[^"]*)"[^}]{0,300}?"Price"\s*:\s*(\d+(?:\.\d+)?)',
        ]
        matches = []
...
def get_prices():
    ...
    logger.info(f"[B2W] {len(results)} ofertas encontradas.")
    return results
