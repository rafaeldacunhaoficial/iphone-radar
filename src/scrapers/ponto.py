"""Ponto (ex-Ponto Frio) scraper – vtexcommercestable subdomain."""
import logging
import requests
from . import _http
...
BASE = "https://ponto.vtexcommercestable.com.br"
...
def _parse_vtex(products: list) -> list:
    results = []
    for p in products:
        ...
        for item in p.get("items") or []:
            for seller in item.get("sellers") or []:
                ...
                if price and float(price) > 500:
                    pid = f"pt_{p.get('productId') or abs(hash(name)) % 9999999}"
                    url = p.get("link") or ""
                    if url and not url.startswith("http"):
                        url = "https://www.ponto.com.br/" + url
                    results.append({
                        "store": "ponto",
                        "model": name[:120],
                        "title": name[:120],
...
def get_prices() -> list[dict]:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            results = _parse_vtex(products)
    except Exception as e:
        logger.warning(f"[ponto] shelf: {e}")
    ...
    if not results:
        ...
        try:
            ...
            logger.warning(f"[ponto] catalog: {e}")
    ...
    logger.info(f"[ponto] {len(deduped)} iPhones via {method}")
    return deduped
